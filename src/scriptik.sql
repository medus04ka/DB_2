...
-- SELECT "43КТиУ"."ГРУППА", "43КТиУ"."КОЛИЧЕСТВО" FROM
...
SELECT "Н_УЧЕНИКИ"."ГРУППА",
       AVG(date_part('year', age(CASE WHEN "Н_ЛЮДИ"."ДАТА_СМЕРТИ" = '9999-09-09' THEN CURRENT_DATE ELSE "Н_ЛЮДИ"."ДАТА_СМЕРТИ" END, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ"))) AS avg_age
FROM "Н_ЛЮДИ"
         JOIN "Н_УЧЕНИКИ" ON "Н_УЧЕНИКИ"."ЧЛВК_ИД" = "Н_ЛЮДИ"."ИД"
GROUP BY "Н_УЧЕНИКИ"."ГРУППА"
HAVING AVG(date_part('year', age(CASE WHEN "Н_ЛЮДИ"."ДАТА_СМЕРТИ" = '9999-09-09' THEN CURRENT_DATE ELSE "Н_ЛЮДИ"."ДАТА_СМЕРТИ" END, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ"))) < (
    SELECT AVG(date_part('year', age(CASE WHEN "Н_ЛЮДИ"."ДАТА_СМЕРТИ" = '9999-09-09' THEN CURRENT_DATE ELSE "Н_ЛЮДИ"."ДАТА_СМЕРТИ" END, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ")))
    FROM "Н_ЛЮДИ"
             JOIN "Н_УЧЕНИКИ" ON "Н_УЧЕНИКИ"."ЧЛВК_ИД" = "Н_ЛЮДИ"."ИД"
    WHERE "Н_УЧЕНИКИ"."ГРУППА" = '1101'
);
...
FROM "Н_УЧЕНИКИ" "ВНЕШ_УЧЕНИКИ"
    ...
    AND "Н_НАПР_СПЕЦ"."НАИМЕНОВАНИЕ" = 'Программная инженерия'
         JOIN "Н_УЧЕНИКИ" "ВНУТР_УЧЕНИКИ" ON "ВНУТР_УЧЕНИКИ"."ИД" = "ВНЕШ_УЧЕНИКИ"."ИД"
    AND "ВНУТР_УЧЕНИКИ"."ПРИЗНАК" = 'отчисл'
...
--              JOIN "Н_НАПР_СПЕЦ" ON "Н_НАПР_СПЕЦ"."ИД" = "Н_НАПРАВЛЕНИЯ_СПЕЦИАЛ"."НС_ИД"
--     WHERE "ВНЕШ_УЧЕНИКИ"."ПЛАН_ИД" = "Н_НАПРАВЛЕНИЯ_СПЕЦИАЛ"."ИД"
--       AND "Н_НАПР_СПЕЦ"."НАИМЕНОВАНИЕ" = 'Программная инженерия'
-- );
